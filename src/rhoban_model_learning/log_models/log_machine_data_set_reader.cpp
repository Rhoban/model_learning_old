...
typedef LogMachineDataSetReader LMDSR;
typedef LogMachineInput LMI;

LMDSR::LogMachineDataSetReader() : nb_training_tags(-1), nb_validation_tags(-1), verbose(false)
{
}

DataSet LMDSR::extractSamples(const std::string& file_path, std::default_random_engine* engine) const
{
  ...
  for (size_t row = 0; row < data.nbRows(); row++)
  {
    ...
    samples_by_corner_id_marker_id_image_id[key] = std::unique_ptr<Sample>(new Sample(
        std::unique_ptr<Input>(new LMI(image_id, marker_id, corner_id, camera_from_self, camera_from_head_base)),
...
std::string LMDSR::getClassName() const
{
  return "LogMachineDataSetReader";
}

Json::Value LMDSR::toJson() const
{
  Json::Value v;
...
void LMDSR::fromJson(const Json::Value& v, const std::string& dir_name)
{
  (void)dir_name;
